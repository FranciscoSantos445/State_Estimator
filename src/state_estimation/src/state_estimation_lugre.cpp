...
//getters
TireInfoStruct const & Lugre::getTireInfo(uint8_t id)const{
    return _tireInfoArray[id];
}
...
LinModelMatrix Lugre::fullCarDynamics(std::vector<Tire> &tireArray, const Matrix<double,4,1> &steerAngle, const Matrix<double,4,1> &wheelSpeedArray, const double vx, const double vy, const double wz, const Vector4d &FnArray, const double Iz, const double m) {
    ...
    const LinModelMatrixTire contStateMatricesFL = tireDynamics(tireArray[0], steerAngle(0), wheelSpeedArray(0), vx, vy, wz, FnArray(0), Iz, m, 0);

    const LinModelMatrixTire contStateMatricesFR = tireDynamics(tireArray[1], steerAngle(1), wheelSpeedArray(1), vx, vy, wz, FnArray(1), Iz, m, 1);

    const LinModelMatrixTire contStateMatricesRL = tireDynamics(tireArray[2], steerAngle(2), wheelSpeedArray(2), vx, vy, wz, FnArray(2), Iz, m, 2);
    
    const LinModelMatrixTire contStateMatricesRR = tireDynamics(tireArray[3], steerAngle(3), wheelSpeedArray(3), vx, vy, wz, FnArray(3), Iz, m, 3);
    ...
    // This is so that the predicted accelerations follow the xsense values
    C(4, 13) -= wz;
    C(5, 12) += wz;
...
LinModelMatrixTire Lugre::tireDynamics(Tire& tire, const double delta, const double w, const double vx, const double vy, const double wz, double Fn, const double Iz, const double m, int tireId) {
    ...
    L = 2.0*sqrtf(fmax(pow(tire.r,2)-pow(rl,2), _eps)); //contact patch length
    ...
    const double Ox = computeOx(tire, rl, w, vr, L, tireId);
    const double Oy = computeOy(tire, rl, w, vr, L, tireId);
...
double Lugre::computeOx(const Tire &tire, const double re, const double w, const Matrix<double,2,1> &vr, const double L, int tireId ) {
    ...
    _tireInfoArray[tireId].Gx = g;
    _tireInfoArray[tireId].Kx = kx;
    _tireInfoArray[tireId].Vrx = vr(0);
    _tireInfoArray[tireId].Vry = vr(1);
    _tireInfoArray[tireId].Ox = Ox;
    return Ox;
}
...
double Lugre::computeOy(const Tire &tire, const double re, const double w, const Matrix<double,2,1> &vr, const double L, int tireId ) {
    ...
    _tireInfoArray[tireId].Gy = g;
    _tireInfoArray[tireId].Oy = Oy;
    return Oy;
}
